    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            for m in net.modules():
                if hasattr(m, 'mask'):
                    # Convert NumPy mask to Torch Tensor and move to the same device as the weights
                    mask_tensor = torch.from_numpy(m.mask).to(device)
                    
                    # Access the internal layer to apply the mask to the gradient
                    if hasattr(m, 'linear'):
                        m.linear.weight.grad.data.mul_(mask_tensor)
                    elif hasattr(m, 'conv'):
                        m.conv.weight.grad.data.mul_(mask_tensor)
