...
class PruneLinear(nn.Module):
    ...
    def prune_by_percentage(self, q=5.0):
        ...
        """
        # Get the absolute value of the weight parameters
        abs_weight = self.linear.weight.data.abs()
        # Get the threshold value by the percentile 'q'
        threshold = np.percentile(abs_weight.cpu().numpy(), q)
        # Create a mask for the weights that are above the threshold
        mask = (abs_weight > threshold).float().to(device)
        # Update the weight parameters directly so that the forward pass will use the pruned weights
        self.linear.weight.data.mul_(mask)
        # Store the mask for further fine-tuning processes
        self.mask = mask.cpu().numpy()
        # Calculate the sparsity after pruning
        self.sparsity = 1.0 - (mask.sum().item() / mask.numel())
    ...
    def prune_by_std(self, s=0.25):
        ...
        will be pruned.
        """
        # Get the raw weights and calculate the absolute value
        weight = self.linear.weight.data
        abs_weight = weight.abs()
        # Get the threshold value by the standard deviation 's'
        threshold = s * np.std(weight.cpu().numpy())
        # Create a mask for the weights that are above the threshold
        mask = (abs_weight > threshold).float().to(device)
        # Update the weight parameters directly so that the forward pass will use the pruned weights
        self.linear.weight.data.mul_(mask)
...
class PrunedConv(nn.Module):
    ...
    def prune_by_percentage(self, q=5.0):
        ...
        """
        # Get the absolute value of the weight parameters
        abs_weight = self.conv.weight.data.abs()
        # Get the threshold value by the percentile 'q'
        threshold = np.percentile(abs_weight.cpu().numpy(), q)
        # Create a mask for the weights that are above the threshold
        mask = (abs_weight > threshold).float().to(device)
